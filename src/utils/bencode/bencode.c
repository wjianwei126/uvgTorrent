...
Hashmap * bencode_to_hashmap(bencode_t * be){
    ...
    if (bencode_is_dict(be))
    {
        while (bencode_dict_has_next(be))
        {
            ...
            if (0 == bencode_dict_get_next(be, &benk, &key, &klen))
                break;


            char dict_key[klen];
            memcpy(&dict_key[0], key, klen);
            dict_key[klen] = '\0';
            ...
            if(bencode_is_dict(&benk)) {
                ...
                hashmap->set(hashmap, dict_key, &int_val, sizeof(long));
            } else if(bencode_is_string(&benk)){
                // printf("STR %s \n", dict_key);
                int len;
                const char *str;
                
                bencode_string_value( &benk, &str, &len );

                char val[len+1];
                memcpy(&val[0], str, len);
                val[len] = '\0';

                hashmap->set(hashmap, dict_key, &val, len);
            } else if(bencode_is_list(&benk)){
                // printf("LIST %s \n", dict_key);
                ...
                while (bencode_list_has_next(&benk))
                {
                    ...
                    if (-1 == bencode_list_get_next(&benk, &benl))
                        printf("fail\n");

                    if(bencode_is_dict(&benl)) {
                        Hashmap * child = bencode_to_hashmap(&benl);
                        list->append(list, child, sizeof(Hashmap));
                        free(child);
                    } else if(bencode_is_string(&benl)){
                        int len;
                        const char *str;
                        
                        bencode_string_value( &benl, &str, &len );

                        char val[len+1];
                        memcpy(&val[0], str, len);
                        val[len] = '\0';

                        list->append(list, &val, len);
                    }
                }

                hashmap->set(hashmap, dict_key, list, sizeof(Linkedlist));
                free(list);
            }
        }
    }
